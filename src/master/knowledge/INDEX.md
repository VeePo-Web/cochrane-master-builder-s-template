...
> **Family-of-sites callout:** the v1.0 Master Plan (`.xlsm`), v2.0 Services & Pricing (`.xlsx`), v4.0 Ideal Customer Brain (`.xlsx`), and v5.0 Color + UX Direction (`.xlsx`) are a **paired set of four, joined on `Master Row`**, and must always be consulted **together** when building a new Cochrane spin-off site. v1.0 supplies *positioning + SEO + linking + launch phase*; v2.0 supplies *services offered + mid-high CAD price ranges + compliance guardrails*; v4.0 supplies *per-site Ideal Customer narrative — voice, emotional drivers, fears, language*; v5.0 supplies *per-site palette + visual mood + UX rationale*. Live `.ca` availability is NOT verified in any of the four sheets — always WHOIS-check before recommending purchase. Drift in `Recommended Domain` between the four files for the same `Master Row` = Sync Discrepancy.
