...
> **Universal base copy manuscript** — the remix base every per-site copy draft branches from. Lives under `partner-documents/.../copy/`. Variables in `[brackets]` are resolved per spin-off using v1.0–v10.0 + communities registry, then polished by the **Master Copywriter** persona and audited by the **Auditor** for banned words, voice, master-message echo, and word counts.

| File | Format | Status | One-line label |
|------|--------|--------|----------------|
| `partner-documents/brands/cochrane-master-builders/copy/universal_base_copy_manuscript.partner.md` | Markdown (partner-only manuscript) | partner-only | **Universal Base Copy Manuscript — the remix base.** Complete homepage end-to-end (Trust Bar → Hero ×3 headline options + subhead + 2 CTAs + trust microcopy → Trust Strip ×4 cards → Services intro + 6 cards → Pricing 3 tiers → Process 5 steps → Legacy Block ×3 headline options + body → Project Proof + card template → Areas We Serve + 5 cluster blocks → 8 starter FAQs → Final CTA ×3 + dual CTA → Footer + sign-off) plus 9 sub-page manuscripts (Services / Pricing-Cost-Guide / Process / Areas We Serve / About / Contact / FAQ / Projects / Blog), 11 service-category copy overlays (Roofing / Tile-Flooring / Decks / Concrete / Basement Suites / Renovations / HVAC / Repairs / Landscaping / Custom Homes / Commercial), microcopy system (buttons / form helpers / error messages / success states / empty states), SEO meta library (title tag patterns / meta description patterns / OpenGraph / schema descriptions), word-count targets, master-message echo plan (≥4 surfaces per site), and a 10-point pre-ship audit checklist. Variables in `[brackets]` for find-and-replace remix. **Keywords:** remix base, universal copy, homepage manuscript, sub-page copy, headline options, subhead, microcopy, buttons, form helpers, error messages, success states, title tags, meta descriptions, word counts, audit checklist, "Strong Foundations For Those Who Come After Us," master message echo, service-category copy overlays, find-and-replace, per-site polish, banned-word audit. |
