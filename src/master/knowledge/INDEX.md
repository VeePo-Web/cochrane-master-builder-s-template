...
> **Family-of-sites callout:** the v1.0 Master Plan (`.xlsm`), v2.0 Services & Pricing (`.xlsx`), v4.0 Ideal Customer Brain (`.xlsx`), v5.0 Color + UX Direction (`.xlsx`), v6.0 UX Experience (`.xlsx`), v7.0 Performance Audit Specs (`.xlsx`), and v9.0 Theme Layout / Bespoke UX Components (`.xlsx`, supersedes v8.0) are a **paired set of seven, joined on `Master Row`**, and must always be consulted **together** when building a new Cochrane spin-off site. v1.0 supplies *positioning + SEO + linking + launch phase*; v2.0 supplies *services offered + mid-high CAD price ranges + compliance guardrails*; v4.0 supplies *per-site Ideal Customer narrative — voice, emotional drivers, fears, language*; v5.0 supplies *per-site palette + visual mood + UX rationale*; v6.0 supplies *per-site UX experience theme + experience strategy — pacing, motion temperament, interaction feel, emotional arc*; v7.0 supplies *per-site performance posture + Core Web Vitals audit spec — speed, weight, asset/render strategy, mobile-first plan*; v9.0 supplies *per-site theme layout + bespoke UX components + perf-aware build notes — page architecture, signature modules, IA scaffolding, cluster-level family resemblance*. Live `.ca` availability is NOT verified in any of the seven sheets — always WHOIS-check before recommending purchase. Drift in `Recommended Domain` between any two of the seven files for the same `Master Row` = Sync Discrepancy.
