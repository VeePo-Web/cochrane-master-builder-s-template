...
> **Family-of-sites callout:** the v1.0 Master Plan (`.xlsm`), v2.0 Services & Pricing (`.xlsx`), and v4.0 Ideal Customer Brain (`.xlsx`) are a **paired set of three, joined on `Master Row`**, and must always be consulted **together** when building a new Cochrane spin-off site. v1.0 supplies *positioning + SEO + linking + launch phase*; v2.0 supplies *services offered + mid-high CAD price ranges + compliance guardrails*; v4.0 supplies *per-site Ideal Customer narrative — voice, emotional drivers, fears, language*. Live `.ca` availability is NOT verified in any of the three sheets — always WHOIS-check before recommending purchase. Drift in `Recommended Domain` between the three files for the same `Master Row` = Sync Discrepancy.
