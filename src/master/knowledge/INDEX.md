...
> **Family-of-sites callout:** the v1.0 Master Plan (`.xlsm`), v2.0 Services & Pricing (`.xlsx`), v4.0 Ideal Customer Brain (`.xlsx`), v5.0 Color + UX Direction (`.xlsx`), v6.0 UX Experience (`.xlsx`), and v7.0 Performance Audit Specs (`.xlsx`) are a **paired set of six, joined on `Master Row`**, and must always be consulted **together** when building a new Cochrane spin-off site. v1.0 supplies *positioning + SEO + linking + launch phase*; v2.0 supplies *services offered + mid-high CAD price ranges + compliance guardrails*; v4.0 supplies *per-site Ideal Customer narrative — voice, emotional drivers, fears, language*; v5.0 supplies *per-site palette + visual mood + UX rationale*; v6.0 supplies *per-site UX experience theme + experience strategy — pacing, motion temperament, interaction feel, emotional arc*; v7.0 supplies *per-site performance posture + Core Web Vitals audit spec — speed, weight, asset/render strategy, mobile-first plan*. Live `.ca` availability is NOT verified in any of the six sheets — always WHOIS-check before recommending purchase. Drift in `Recommended Domain` between any two of the six files for the same `Master Row` = Sync Discrepancy.
