# Partner Document — 8.0 Cochrane Masters · All-115 Theme Layout / Bespoke UX Components / Performance Optimization

**Source (verbatim):** `src/master/knowledge/source-documents/brands/cochrane-master-builders/strategy/8.0_cochrane_masters_all_115_theme_layout_ux_specs.xlsx`
**Markdown mirror (1:1):** `…/8.0_cochrane_masters_all_115_theme_layout_ux_specs.source.md`

## Title
Cochrane Masters — All-115 Theme Layout / Bespoke UX Components Companion (v8.0)

## Category
brands → cochrane-master-builders → strategy → per-site **page architecture + bespoke component recipe + perf-aware build notes** (seventh file in the paired set).

## Main Purpose
Canonical per-site **theme layout + bespoke UX component recipe + perf-aware build notes** for all 115 planned Cochrane spin-off sites. The "page architecture and signature components" leg of the remix **heptapod**:
...
- **v8.0 (this file)** — Per-site **theme layout + bespoke UX components + perf-aware build notes** (page architecture, signature modules, IA scaffolding)

All seven join on `Master Row`. Drift in `Recommended Domain` between any two = **Sync Discrepancy**.
...
- **Join discipline:** any per-site work must reference the same `Master Row` across all seven files.
